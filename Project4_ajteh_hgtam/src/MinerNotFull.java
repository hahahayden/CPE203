...
public class MinerNotFull extends Miner {
    ...
    private boolean entityExists = false;
    private boolean fireBlobExists=false;
...
//        checkVolcano(world,imageStore);
        Background minerBackgroundCell = new Background("lava", imageStore.getImageList("lava"));
        if (world.getBackgroundCell(this.position).equals(minerBackgroundCell)) {

                setImageList(imageStore.getImageList("possessedMiner"));
//            this.actionPeriod = this.actionPeriod /25;
          this.animationPeriod = this.animationPeriod /2;

             setactionPeriod(this.actionPeriod/5);
            if (!fireBlobExists)
            {
                if (!world.isOccupied(new Point(this.position.getX(),this.position.getY()-1)))
                {
                    Entity fireblob = createFactory.createFireBlob("FireBlob", new Point(this.position.getX(), this.position.getY()-1), 10, 5, imageStore.getImageList("FireBlob"));
                    world.addEntity(fireblob);
                    ((ActionEntity) fireblob).scheduleActions(scheduler, world, imageStore);
                }
            }
            else{
                fireBlobExists=true;
