...
public class AStarPathingStrategy extends SingleStepPathingStrategy {
    ...
    public List<Point> computePath(Point start, Point end,
        ...
        PriorityQueue<Node> openNodes = priorityQueue();   // create a priority queue
        ...
        Node endNode = new Node(end, -1, -1, null);   // only needed to check if the last point is reached; could honestly jsut use Point end but looks neater
        ...
        while (!openNodes.isEmpty()) {
            ...
            for (Point p : neighbors) {
                ...
                if (testNode != null && nextNode.getfDist() >= testNode.getfDist())   // check if within adn equal distances  but im pretty sure the queue already deals
                    continue;
            ...
            closedList.put(current.getPosition(), current);  // when lowest neighbor found per tile go back and put that currrent into close list
        }
        return path;
    }
    ...
    public List<Point> reconstructPath(Node endNode, List<Point> myPath) { // back track from the getPrior; no need to use the closed list
        if (endNode == null)
            return myPath;
