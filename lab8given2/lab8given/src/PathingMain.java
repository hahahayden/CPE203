...
//		return false;
//   }
   private boolean moveOnceDFS( Point pos, GridValues [][] grid, List<Point> path) {
...
//            Point down = new Point(pos.x, pos.y + 1);
            ...
            flag = moveOnceDFS(rightN, grid, path) || moveOnceDFS(left, grid, path) || moveOnceDFS(down, grid, path) || moveOnceDFS(up, grid, path);
         }
         if (flag) {
            this.path.add(pos);
         }
